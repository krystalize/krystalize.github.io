...
function checkHexDigit(){
	var total = 0;
	var nums = "";
	var finalString = "";
		for (var i = 8; i > 0; i--){
			var hexvalue = 0;
			if (document.getElementById("bit"+i).value == "A"){
				hexvalue = 10;
				alert(hexvalue);
			}else if(document.getElementById("bit"+i).value == "B"){
				hexvalue = 11;
				alert(hexvalue);
			}else if(document.getElementById("bit"+i).value == "C"){
				hexvalue = 12;
				alert(hexvalue);
			}else if(document.getElementById("bit"+i).value == "D"){
				hexvalue = 13;
				alert(hexvalue);
			}else if(document.getElementById("bit"+i).value == "E"){
				hexvalue = 14;
				alert(hexvalue);
			}else if(document.getElementById("bit"+i).value == "F"){
				hexvalue = 15;
				alert(hexvalue);
			}else{
				hexvalue = document.getElementById("bit"+i).value;
				alert(hexvalue);
			}
			if (document.getElementById("bit"+i).value != "0"){
				document.getElementById("value"+i).style.backgroundColor = "#3bde2c";
				total = total + Math.pow(hexvalue,i-1);
				nums = nums.concat(Math.pow(Math.pow(16, i-1),hexvalue),"+");
			}
